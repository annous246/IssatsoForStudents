...
let double_save=(app,timetablesmodel,errorChecker)=>{
    app.post('/double_save',async(req,res)=>{
      if(!req.session.user){
        res.redirect('/home')
      }
        ...
        count=1
          ...
          if(ok){
            ...
            await timetablesmodel.findOneAndUpdate({id:`A0${classA}G1`},{table:olddata.table})
            .then((d)=>{console.log("schedule updated");return d})
            .catch(()=>console.log("error updating schedule"))
            await timetablesmodel.findOneAndUpdate({id:`A0${classA}G2`},{table:olddata.table})
            .then((d)=>{console.log("schedule updated");return d})
            .catch(()=>console.log("error updating schedule"))
          }
          ...
          let obj={id:`A0${classA}G2`,group:1,table:table}
          let obj2={id:`A0${classA}G1`,group:1,table:table}
          let tobesaved=new timetablesmodel(obj)
          tobesaved.save()
          .then(()=>console.log("saved"))
          .catch("error saving new table");
    let tobesaved2=new timetablesmodel(obj2)
          tobesaved.save()
          .then(()=>console.log("saved"))
          .catch("error saving new table");
